...
SIMPLE_JWT = {
    ...
    'ROTATE_REFRESH_TOKENS': False,  # Không tự động xoay token làm mới khi yêu cầu
    'BLACKLIST_AFTER_ROTATION': False,  # Không blacklist token làm mới sau khi xoay
    'ALGORITHM': 'HS256',  # Thuật toán mã hóa được sử dụng cho JWT (HS256 là phổ biến)
    'SIGNING_KEY': 'your-secret-key',  # Khóa bí mật để ký token
    'VERIFYING_KEY': None,  # Nếu bạn cần sử dụng khóa công khai để xác thực token (optional)
    # Thêm vô
